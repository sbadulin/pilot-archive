...
Работать в ветках, открывать pull request в `main`. Проверка `Tests and builds` запускает TypeScript, тесты, проверку каталога архива и обе сборки. После merge `Deploy production` повторяет проверку и публикует frontend в Selectel S3, админку — в Pages.
...
Публикация выпусков — отдельный процесс backend. Workflow интерфейса не отправляет PDF и не применяет `--delete` к бакету. Существующие PDF, каталог и старые хешированные JS остаются на месте. Каталог и бинарные файлы хранятся в Selectel S3; workflow не перезаписывает живой manifest локальной копией.
