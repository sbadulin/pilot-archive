...
function PdfCanvas({ pdf, page, width = 900, rotation = 0 }: { pdf: PDFDocumentProxy; page: number; width?: number; rotation?: number }) {
    ...
    const render = async () => {
      try {
        ...
        if (!cancelled && (e as Error).name !== 'RenderingCancelledException') {
          setError('Не удалось показать лист. Выберите его ещё раз.');
          setLoading(false);
        }
      }
    };
  ...
  return <div ref={container} className="canvas-wrap" style={{ width, aspectRatio: ratio }} aria-busy={loading}>
    {loading && <div className="render-status" role="status">Открываем лист…</div>}
    {error && <p role="alert">{error}</p>}
    <canvas ref={canvas} aria-label={`Лист ${page}`} style={{ width: '100%', height: 'auto' }} />
  </div>;
}
...
function Reader({ issue, onBack, initialPage = 1 }: { issue: Issue; onBack: () => void; initialPage?: number }) {
  ...
  return <main className="reader">
    ...
    <div className="reader-toolbar">
      <button className={`tool-button ${showPages ? 'pressed' : ''}`} aria-label="Показать миниатюры листов" aria-pressed={showPages} onClick={() => setShowPages(!showPages)}><Grid2X2 size={18} /></button>
      <div className="page-control">
        <button className="tool-button" aria-label="Предыдущий лист" disabled={page === 1} onClick={() => goPage(page - 1)}><ChevronLeft size={20} /></button>
        <label>Лист <input aria-label="Номер листа" type="number" min={1} max={issue.pages} value={page} onChange={e => { if (e.target.value) goPage(Number(e.target.value)); }} /> из {issue.pages}</label>
        <button className="tool-button" aria-label="Следующий лист" disabled={page === issue.pages} onClick={() => goPage(page + 1)}><ChevronRight size={20} /></button>
      </div>
      ...
      <button className="tool-button" aria-label="Повернуть лист" title="Повернуть лист" onClick={() => setRotation(v => (v + 90) % 360)}><RotateCw size={18} /></button>
      <div className="view-mode" role="group" aria-label="Режим просмотра"><button className={viewMode === 'sheet' ? 'active' : ''} onClick={() => setViewMode('sheet')}>Лист</button><button className={viewMode === 'continuous' ? 'active' : ''} onClick={() => setViewMode('continuous')}>Лента</button><button className={viewMode === 'spreads' ? 'active' : ''} onClick={() => setViewMode('spreads')}>Развороты</button></div>
    </div>
    <div className="reader-body">
      {showPages && <nav className="thumbnails" aria-label="Листы выпуска">{Array.from({ length: issue.pages }, (_, i) => i + 1).map(n => <button key={n} className={`thumbnail ${page === n ? 'selected' : ''}`} aria-current={page === n ? 'page' : undefined} onClick={() => goPage(n)}>{pdf ? <PdfCanvas pdf={pdf} page={n} width={110} /> : <span className="page-placeholder">{n}</span>}<span>{n === 1 ? '1 · Обложка' : `Лист ${n}`}</span></button>)}</nav>}
      <div className={`sheet-viewport view-${viewMode}`} ref={viewport}>
        {viewMode === 'sheet' && <><div className="sheet-info" role="status">{largePages.includes(page) ? 'Увеличенный лист' : page === 1 ? 'Обложка' : `Лист ${page}`}<span>Исходный PDF</span></div><div className="sheet-inner" style={{ minWidth: baseWidth * zoom / 100 + 48 }}>{pdf ? <PdfCanvas pdf={pdf} page={page} width={baseWidth * zoom / 100} rotation={rotation} /> : <p className="pdf-loading">Открываем PDF…</p>}</div></>}
        {viewMode === 'continuous' && <div className="continuous-pages">{Array.from({ length: issue.pages }, (_, index) => index + 1).map(number => <figure ref={node => { pageNodes.current[number] = node; }} className={largePages.includes(number) ? 'large-page' : ''} key={number}><figcaption>{largePages.includes(number) ? `Лист ${number} · увеличенный` : `Лист ${number}`}</figcaption>{pdf ? <PdfCanvas pdf={pdf} page={number} width={baseWidth * zoom / 100} /> : <p className="pdf-loading">Открываем PDF…</p>}</figure>)}</div>}
    ...
    <footer className="reader-footer"><span>{issue.pages} листов в PDF.</span><span>← → листать <span className="shortcut">Esc закрыть</span></span></footer>
  </main>;
}
  ...
  const loadFile = async (file: File) => {
    ...
    try {
      ...
      for (let n = 1; n <= pdf.numPages; n++) {
        if (current !== run.current) { void pdf.destroy(); return; }
        setBusy(`Готовим листы: ${n} из ${pdf.numPages}`);
      ...
      void pdf?.destroy();
      if (current === run.current) setError((e as Error).message === 'too-many-pages' ? 'Можно проверить PDF до 100 листов.' : 'Не удалось открыть PDF. Проверьте, что файл не повреждён и не защищён паролем, или выберите другой.');
...
export default function Home() {
  ...
  return <>
    ...
    {screen.kind === 'upload' ? <Upload onBack={back} /> : <main id="main" className="shell archive-page">
      ...
      <section className="creators-section" id="creators" aria-labelledby="creators-title"><div className="creators-heading"><span className="black-label">Люди «Пилота»</span><h2 id="creators-title">Газету делали<br />сами люди.</h2><p>Имена людей, связанных с газетой. Состав постепенно уточняем по сохранившимся номерам и воспоминаниям.</p></div><div className="creator-list"><article><strong>Олеся Луконина</strong></article><article><strong>Константин Луконин</strong></article><article><strong>Павел Трутнев</strong></article><article><strong>Александр Бочкарев</strong></article><article><strong>Вадим Горуленко</strong></article><article><strong>Ульяна Крикливая</strong></article><article><strong>Алексей Ларин</strong></article><article><strong>Светлана Гулай</strong></article><article><strong>Наталья Останина</strong></article><article><strong>Ольга Харламова</strong></article><article><strong>Анна Сидорова</strong></article><article><strong>Сергей Долженко</strong></article><article><strong>Сергей Бадулин</strong></article><article><strong>Марина Бабич</strong></article><article><strong>Кирилл Маковеев</strong></article><article><strong>Валерий Кузьмин</strong></article><article><strong>Александр Утюпин</strong></article><article><strong>Ирина Луконина</strong></article><article><strong>Леонид Шевченко</strong></article><article><strong>Евгения Белоусова</strong></article></div></section>
    </main>}
