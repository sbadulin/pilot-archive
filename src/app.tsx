...
const issueRoute = (issue: Issue) =>
  `#issue-${issue.year}-${issue.number}-${issue.serial}`;
...
function Brand({small = false, year = 2000}: {small?: boolean; year?: number}) {
  return (
    <span className={small ? "brand brand-small" : "brand"}>
      <img src={logoForYear(year)} alt="Первый Пилот" />
    </span>
  );
}
  ...
  useEffect(() => {
    ...
    const render = async () => {
      try {
        ...
        if (cancelled) return;
        const base = sheet.getViewport({
          scale: 1,
          rotation: (sheet.rotate + rotation) % 360,
        });
        const viewport = sheet.getViewport({
          scale:
            Math.min(width * (window.devicePixelRatio || 1), 3000) / base.width,
          rotation: (sheet.rotate + rotation) % 360,
        });
...
function dataUrlToBlob(value: string) {
  ...
  return new Blob([bytes], {
    type: header.match(/data:([^;]+)/)?.[1] ?? "image/jpeg",
  });
}
...
function Reader({
  ...
  const [viewMode, setViewMode] = useState<"sheet" | "continuous" | "spreads">(
    "sheet",
  );
    ...
    (next: number) => {
      ...
      if (viewMode === "sheet") viewport.current?.scrollTo(0, 0);
        ...
        requestAnimationFrame(() =>
          pageNodes.current[target]?.scrollIntoView({
            behavior: "smooth",
            block: "start",
          }),
        );
  ...
  useEffect(() => {
    ...
    void openPdf(issuePdfUrl(issue))
      .then(async (result) => {
        document = result;
        if (disposed) void result.destroy();
        else {
          setPdf(result);
          const areas: number[] = [];
          ...
          const detected = areas.flatMap((area, index) =>
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() =>
      setBaseWidth(Math.max(240, Math.min(920, element.clientWidth - 48))),
    );
  ...
  return (
    <main className="reader">
      ...
      <div className="reader-toolbar">
        <button
          className={`tool-button ${showPages ? "pressed" : ""}`}
          aria-label="Показать миниатюры страниц"
        ...
        <div className="view-mode" role="group" aria-label="Режим просмотра">
          <button
          ...
          <button
            className={viewMode === "spreads" ? "active" : ""}
      ...
      <div className="reader-body">
        {showPages && (
          <nav className="thumbnails" aria-label="Страницы выпуска">
            {Array.from({length: issue.pages}, (_, i) => i + 1).map((n) => (
              <button
                key={n}
                className={`thumbnail ${page === n ? "selected" : ""}`}
        ...
        <div className={`sheet-viewport view-${viewMode}`} ref={viewport}>
          ...
          {viewMode === "sheet" && (
            <>
              ...
              <div
                ...
                style={{minWidth: (baseWidth * zoom) / 100 + 48}}
              >
                {pdf ? (
                  <PdfCanvas
                    pdf={pdf}
                  ...
                  <p className="pdf-loading">Открываем PDF…</p>
                )}
              </div>
            </>
          )}
          {viewMode === "continuous" && (
            <div className="continuous-pages">
              {Array.from({length: issue.pages}, (_, index) => index + 1).map(
                (number) => (
                  <figure
                    ref={(node) => {
                      pageNodes.current[number] = node;
                    }}
                    className={largePages.includes(number) ? "large-page" : ""}
                    key={number}
                  >
                    <figcaption>
                      {pageCaption(number)}
            ...
            <div className="spread-pages">
              {spreadGroups.map((pages) => (
                <div
                  ...
                  className={`spread ${pages.length === 1 ? "single" : ""} ${pages.length === 1 && largePages.includes(pages[0]) ? "large" : ""}`}
                  key={pages[0]}
                >
                  {pages.map((sheet) =>
                    pdf ? (
                      <PdfCanvas
                        ...
                        width={
                          (baseWidth * zoom) /
                          (pages.length === 1 && largePages.includes(pages[0])
                            ? 100
                            : 200)
                        }
                      />
                    ) : (
                      <p key={sheet} className="pdf-loading">
                        Открываем PDF…
                      </p>
                    ),
                  )}
          ...
          {pdfError && (
            <p className="reader-note" role="status">
              Не удалось открыть PDF. Попробуйте обновить страницу или скачать
              файл.
            </p>
          )}
          </div>
      ...
      <footer className="reader-footer">
        <span>{issue.pages} страниц в PDF.</span>
        <span>
          ← → листать <span className="shortcut">Esc закрыть</span>
        </span>
      </footer>
    </main>
  );
...
function Upload({onBack}: {onBack: () => void}) {
  ...
  const loadFile = async (file: File) => {
    ...
    if (!file.name.toLowerCase().endsWith(".pdf")) {
      setError(
        "Выберите файл PDF. Изображения и другие файлы сюда не подходят.",
      );
      return;
    }
    if (file.size > 50 * 1024 * 1024) {
      setError("Файл больше 50 МБ. Выберите PDF меньшего размера.");
      return;
    }
    if (!file.size) {
      setError("Этот файл пустой. Выберите другой PDF.");
      return;
    }
    ...
    try {
      ...
      const typical = [...dimensions].sort((a, b) => a - b)[
        Math.floor(dimensions.length / 2)
      ];
      const next = {
        file,
        pdf,
        thumbnails,
        largePages: dimensions.flatMap((size, index) =>
          size > typical * 1.3 ? [index + 1] : [],
        ),
      };
      void draftRef.current?.pdf.destroy();
      ...
      if (current === run.current)
        setError(
          (e as Error).message === "too-many-pages"
            ? "Можно проверить PDF до 100 страниц."
            : "Не удалось открыть PDF. Проверьте, что файл не повреждён и не защищён паролем, или выберите другой.",
        );
    } finally {
      if (current === run.current) setBusy("");
    }
  };
  const loadExample = async () => {
    setError("");
    setBusy("Открываем первый выпуск…");
    try {
      const result = await fetch(issuePdfUrl(staticIssues[0]));
      if (!result.ok) throw new Error("download");
      await loadFile(
        new File([await result.blob()], staticIssues[0].filename, {
          type: "application/pdf",
        }),
      );
    } catch {
      setBusy("");
      setError("Пример не загрузился. Выберите PDF с вашего устройства.");
    }
  };
  const duplicate = staticIssues.find(
    (issue) =>
      issue.date === metadata.date &&
      Number(issue.number) === Number(metadata.number),
  );
    ...
    try {
      await sendSubmission(
        draft.file,
        {
          ...
          date: metadata.date,
          dateLabel: new Date(`${metadata.date}T12:00:00`).toLocaleDateString(
            "ru-RU",
            {day: "numeric", month: "long", year: "numeric"},
          ),
          pages: draft.pdf.numPages,
        },
        draft.thumbnails[0] ? dataUrlToBlob(draft.thumbnails[0]) : undefined,
      );
      setSubmissionMessage(
        "Заявка отправлена куратору. Выпуск появится в архиве после проверки.",
      );
      setDone(true);
      if (isCurator)
        void loadSubmissionQueue()
          .then((result) => setQueue(result.submissions))
          .catch(() => undefined);
    } catch (error) {
  ...
  const openPreview = async (item: Submission) => {
    ...
    try {
      ...
      setPreview({item, pdf, thumbnails});
      setPreviewPage(1);
    } catch (error) {
  ...
  return (
    <main id="main" className="upload-page shell">
      ...
      <div className="upload-heading">
        <div>
          <p className="eyebrow">Пополняем архив вместе</p>
          <h1>Добавить выпуск</h1>
        </div>
      </div>
      <p className="upload-intro">
        Войдите перед отправкой, затем выберите PDF, сверьте обложку и проверьте
        страницы. После отправки файл попадёт куратору на проверку.
      </p>
      <div className="button-row access-actions">
        <a className="outline-button" href="/api/admin/login?return=%2F%23add">
          Войти
        </a>
        {isCurator && (
          <button
          ...
          >
            Показать очередь куратору
          </button>
        )}
      </div>
      <ol className="steps" aria-label="Шаги добавления">
        {["Выбрать PDF", "Проверить данные", "Посмотреть страницы"].map(
          (title, i) => (
            <li
              key={title}
              className={
            ...
            >
              <span>{step > i + 1 || done ? <Check size={16} /> : i + 1}</span>
              {title}
            </li>
          ),
        )}
      ...
      {done && draft ? (
        <section className="completion" aria-live="polite">
          ...
          <p>
            № {metadata.number}
            {metadata.serial && ` (${metadata.serial})`} от{" "}
            {new Date(`${metadata.date}T12:00:00`).toLocaleDateString("ru-RU")}.
            Страниц: {draft.pdf.numPages}.
          </p>
          <p>
            {submissionMessage ||
              "Выпуск передан куратору. До проверки он не появится в публичном архиве."}
          </p>
        ...
        <>
          {step === 1 && (
            <section className="upload-start">
              <div
                ...
                onDrop={(e) => {
                  ...
                  if (e.dataTransfer.files.length !== 1) {
                    setError("Добавляйте по одному PDF за раз.");
                    return;
                  }
                  void loadFile(e.dataTransfer.files[0]);
                }}
              >
                <FileUp size={42} strokeWidth={1.3} />
                <h2>Перетащите PDF сюда</h2>
                <p>или выберите его на вашем устройстве</p>
                <button
                ...
                <span className="muted">Один файл, до 50 МБ</span>
                <input
                  ref={input}
                  type="file"
              ...
              <aside className="upload-help">
                <span className="black-label">Под рукой</span>
                <h3>Весь выпуск одним файлом</h3>
                <p>Подойдёт PDF со сканами или исходный PDF из редакции.</p>
                <p>Большие страницы оставьте в исходном размере.</p>
                <div className="try-example">
                  <h3>Сначала попробовать?</h3>
                  <p>Пройдите шаги на выпуске от 5 января 2000 года.</p>
                  <button
          ...
          {step === 2 && draft && (
            <section className="metadata-step">
              <div className="cover-check">
                <img
                  src={draft.thumbnails[0]}
                  alt="Первая страница добавленного PDF для проверки даты и номера"
                />
                <span>Обложка загруженного файла</span>
              </div>
              <form
                className="metadata-form"
                onSubmit={(e) => {
                  e.preventDefault();
                  const message = validateMetadata(
                    metadata.date,
                    metadata.number,
                    metadata.serial,
                  );
              ...
              >
                <span className="black-label">Сверьте с обложкой</span>
                <h2>Всё верно?</h2>
                <p>
                  Если дата и номер были в имени файла, мы подставили их.
                ...
                <div className="field-pair">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetadata({...metadata, number: e.target.value})
                      }
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMetadata({...metadata, serial: e.target.value})
                      }
                    />
                    <small>В скобках, если указан</small>
                  </label>
                </div>
                <div className="file-summary">
                  ...
                  <div>
                    <b>{draft.file.name}</b>
                    <span>
                      {(draft.file.size / 1024 / 1024).toLocaleString("ru-RU", {
                        maximumFractionDigits: 1,
                      })}{" "}
                      МБ · Страниц: {draft.pdf.numPages}
                    </span>
                  </div>
                </div>
                {duplicate && (
                  <p className="notice">
                    Этот выпуск уже есть в архиве. Вы можете продолжить
                    проверку, копия в архив не добавится.
                  </p>
                )}
                <div className="button-row">
          ...
          {step === 3 && draft && (
            <section className="review-step">
              <div className="review-topline">
                <div>
                  <h2>Проверьте страницы</h2>
                  <p>
                    Скан читается, страницы идут по порядку и ничего не
                    обрезано.
                  </p>
                </div>
                <span className="sheet-count">
                  {page} / {draft.pdf.numPages}
                </span>
              </div>
              {draft.largePages.length > 0 && (
                <p className="notice">
                  Увеличенный размер у страниц: {draft.largePages.join(", ")}.
                  Это допустимо, проверьте, что они целиком попали в скан.
                </p>
              )}
              <div className="review-workspace">
                <nav
                  className="review-thumbnails"
                  aria-label="Страницы добавленного PDF"
                >
                  {draft.thumbnails.map((src, index) => (
                    <button
                      key={index}
                      className={`thumbnail ${page === index + 1 ? "selected" : ""}`}
                      aria-label={`Посмотреть страницу ${index + 1}`}
                    ...
                    >
                      ...
                      <span>
                        {index + 1}
                        {draft.largePages.includes(index + 1)
                          ? " · большой"
                          : ""}
                      </span>
                    </button>
                ...
                <div className="review-canvas">
                  <PdfCanvas pdf={draft.pdf} page={page} width={700} />
                </div>
              </div>
              <div className="review-pagination">
                ...
                <span>
                  Страница {page} из {draft.pdf.numPages}
                </span>
                <button
                  className="outline-button"
                  disabled={page >= draft.pdf.numPages}
                  onClick={() => setPage((v) => v + 1)}
                >
                  Следующий
              ...
              <label className="confirm-label">
                ...
                />
                Я проверил(а) дату, номер и страницы выпуска
              </label>
              <div className="button-row">
                <button
                  className="primary-button"
              ...
              <p className="local-only">
                PDF попадёт в защищённую очередь на проверку. В публичном архиве
                выпуск появится после одобрения куратором.
              </p>
            </section>
          )}
          {preview && (
            <section
              className="review-step submission-preview"
              aria-labelledby="submission-preview-title"
            >
              <div className="review-topline">
                <div>
                  <span className="black-label">Предпросмотр заявки</span>
                  <h2 id="submission-preview-title">
                    № {preview.item.number} ({preview.item.serial}) ·{" "}
                    {preview.item.year}
                  </h2>
                  <p>{preview.item.filename}</p>
                </div>
                <button
                  className="text-button"
                  type="button"
              ...
              <div className="review-workspace">
                <nav
                  className="review-thumbnails"
                  aria-label="Страницы PDF заявки"
                >
                  {preview.thumbnails.map((src, index) => (
                    <button
                      key={index}
                      className={`thumbnail ${previewPage === index + 1 ? "selected" : ""}`}
                ...
                <div className="review-canvas">
                  <PdfCanvas pdf={preview.pdf} page={previewPage} width={700} />
                </div>
              </div>
              <div className="review-pagination">
                <button
                  ...
                  onClick={() => setPreviewPage((v) => v - 1)}
                >
                  Предыдущий
                </button>
                <span>
                  Страница {previewPage} из {preview.pdf.numPages}
                </span>
                <button
                  className="outline-button"
                  disabled={previewPage >= preview.pdf.numPages}
                  onClick={() => setPreviewPage((v) => v + 1)}
                >
                  Следующий
                </button>
          ...
          {queue && (
            <section className="submission-queue" aria-labelledby="queue-title">
              <span className="black-label">Только для кураторов</span>
              <h2 id="queue-title">Очередь заявок</h2>
              {queueError && (
                  ...
                  {queue.map((item) => (
                    <article className="queue-item" key={item.id}>
                      <div className="queue-item-main">
                        <strong>
                          № {item.number} ({item.serial}) · {item.year}
                        </strong>
                        <span>{item.dateLabel}</span>
                        <span>{item.pages} страниц</span>
                      </div>
                      <div className="queue-item-file">
...
export default function Home() {
  ...
  useEffect(() => {
    void fetch(manifestUrl(), {cache: "no-store"})
      ...
      .then((payload) => {
        ...
        const dynamic = payload.issues.filter(
          (item: Issue) =>
            item &&
            ...
            /^(https:\/\/|\/(?!\/))/.test(item.pdfUrl),
        );
        const merged = new Map(
          staticIssues.map((item) => [
            `${item.year}/${Number(item.number)}/${Number(item.serial)}`,
            item,
          ]),
        );
  ...
  const navigate = useCallback(
    (next: Screen) => {
      if (next.kind === "upload" && isPublicSite()) {
      ...
      const issue =
        next.kind === "reader"
          ? archiveIssues.find((item) => item.id === next.id)
          : undefined;
      window.location.hash =
        next.kind === "reader" && issue
          ? issueRoute(issue)
          : next.kind === "upload"
            ? "add"
            : "archive";
  ...
  useEffect(() => {
    const update = () => {
      ...
      const issueMatch = hash.match(
        /^#issue-(\d{4})-(\d{1,3})-(\d{4,5})(?:-p(\d+))?$/,
      );
      const yearMatch = hash.match(/^#year-(\d{4})$/);
      const linkedIssue = issueMatch
        ? archiveIssues.find(
            (issue) =>
              issue.year === Number(issueMatch[1]) &&
              issue.number === issueMatch[2].padStart(2, "0") &&
      ...
      if (hash.startsWith("#add")) {
        ...
        setYear(linkedIssue.year);
        setScreen({
          kind: "reader",
          id: linkedIssue.id,
          page: issueMatch?.[4] ? Number(issueMatch[4]) : 1,
        });
      } else {
        setScreen({kind: "archive"});
        if (yearMatch && years.includes(Number(yearMatch[1])))
          setYear(Number(yearMatch[1]));
      }
      if (hash === "#about" || hash === "#creators")
        requestAnimationFrame(() =>
          document
  ...
  useEffect(() => {
    const activeIssue =
      screen.kind === "reader"
        ? archiveIssues.find((item) => item.id === screen.id)
        : undefined;
    document.title =
      screen.kind === "reader"
        ? `№ ${activeIssue?.number ?? ""} · Первый Пилот`
        : screen.kind === "upload"
          ? "Добавить выпуск · Первый Пилот"
          : "Первый Пилот · Архив газеты 1996–2007";
  }, [screen]);
  if (screen.kind === "reader")
    return (
      <Reader
        key={screen.id}
        issue={
          archiveIssues.find((issue) => issue.id === screen.id) ??
          archiveIssues[0]
        }
    ...
    <>
      ...
      <header className="site-header shell">
        <nav aria-label="Основная навигация">
          <a
            href="#archive"
            className={screen.kind === "archive" ? "active" : ""}
          >
            Архив
          </a>
          <a
            href="#about"
            onClick={() => {
              if (screen.kind !== "archive") setScreen({kind: "archive"});
            }}
          >
            О газете
          </a>
        ...
        <main id="main" className="shell archive-page">
          <section className="masthead">
            ...
            <div className="masthead-description">
              <span className="black-label">Электронный архив</span>
              <p>
                Молодёжная газета
                <br />
                Комсомольска-на-Амуре
              </p>
              <span className="archive-dates">1996–2007</span>
            </div>
          </section>
          ...
          <nav className="year-picker" aria-label="Выбрать год">
            {years.map((value) => (
              <a
                key={value}
                className={`${year === value ? "selected " : ""}${archiveIssues.some((issue) => issue.year === value) ? "has-issues" : ""}`}
                href={yearRoute(value)}
          ...
          <section
            ...
            aria-label={`Выпуски за ${year} год`}
          >
            <aside className="year-summary">
              <span className="black-label">Год за годом</span>
              <h2>{year}</h2>
              <p>
                {year === 2000
                  ...
                  : year === 2002
                    ? "Декабрь 2002 года."
                    : "Этот год ещё ждёт своих выпусков."}
              </p>
              <div className="year-total">
                {archiveIssues.filter((issue) => issue.year === year).length
                  ? issueCountLabel(
                      archiveIssues.filter((issue) => issue.year === year)
                        .length,
                    )
                  : "Нет загруженных выпусков"}
              </div>
              <div className="archive-note">
                ...
                <span>
                  Обложки и все страницы
                  <br />в исходных материалах выпуска.
                </span>
              </div>
            </aside>
            {archiveIssues.some((issue) => issue.year === year) ? (
                  ...
                  .map((issue) => (
                    <article className="issue-card" key={issue.id}>
                      <button
                        ...
                        onClick={() => navigate({kind: "reader", id: issue.id})}
                        aria-label={`Читать выпуск № ${issue.number} от ${issue.dateLabel}`}
                      >
                        <div className="cover-frame">
                          <img
                            src={scanUrl(issue, 1)}
                            alt={`Обложка газеты Первый Пилот № ${issue.number} (${issue.serial}), ${issue.dateLabel}`}
                            width={issue.id === 1 ? 892 : 925}
                            height={1300}
                            fetchPriority="high"
                          />
                      ...
                      <div className="issue-meta">
                        ...
                        <a
                          ...
                          href={issueRoute(issue)}
                          aria-label={`Открыть выпуск № ${issue.number}`}
                        >
                          <ArrowRight size={24} />
                        </a>
                        <time dateTime={issue.date}>{issue.dateLabel}</time>
                        <span className="issue-format">
                          PDF · {issue.pages} страниц
                        </span>
                      </div>
                    </article>
                  ))}
              ...
              <div className="empty-year">
                ...
                <h3>За {year} год пока пусто</h3>
                <p>
                  В архиве пока нет оцифрованных выпусков за этот год. Если у
                  вас есть другие сканы, попробуйте форму добавления.
                </p>
                <div className="button-row">
                  <button
                    className="primary-button"
          ...
          <section className="about-section" id="about">
            <div>
              <span className="black-label">О газете</span>
              <h2>
                Свой город. <br />
                Своё поколение.
              </h2>
            ...
            <div>
              <p>
                «Первый Пилот» — молодёжная газета Комсомольска-на-Амуре. Здесь
                собираем выпуски за 1996–2007 годы, чтобы их снова можно было
                читать.
              </p>
              <p>
                В выпусках газеты: городские новости, музыка, кино, письма
                читателей и телепрограмма. Газета остаётся целиком — вместе с
          ...
          >
            <div className="creators-heading">
              <span className="black-label">Люди «Пилота»</span>
              <h2 id="creators-title">
                Газету делали
                <br />
                сами люди.
              </h2>
              <p>
                Имена людей, связанных с газетой. Состав постепенно уточняем по
                сохранившимся номерам и воспоминаниям.
              </p>
            </div>
            <div className="creator-list">
              <article>
                <strong>Олеся Луконина</strong>
              </article>
              <article>
                <strong>Константин Луконин</strong>
              </article>
              <article>
                <strong>Павел Трутнев</strong>
              </article>
              <article>
                <strong>Александр Бочкарев</strong>
              </article>
              <article>
                <strong>Вадим Горуленко</strong>
              </article>
              <article>
                <strong>Ульяна Крикливая</strong>
              </article>
              <article>
                <strong>Алексей Ларин</strong>
              </article>
              <article>
                <strong>Светлана Безрукова</strong>
              </article>
              <article>
                <strong>Наталья Останина</strong>
              </article>
              <article>
                <strong>Ольга Харламова</strong>
              </article>
              <article>
                <strong>Анна Сидорова</strong>
              </article>
              <article>
                <strong>Сергей Долженко</strong>
              </article>
              <article>
                <strong>Сергей Бадулин</strong>
              </article>
              <article>
                <strong>Марина Бабич</strong>
              </article>
              <article>
                <strong>Кирилл Маковеев</strong>
              </article>
              <article>
                <strong>Александр Утюпин</strong>
              </article>
              <article>
                <strong>Ирина Луконина</strong>
              </article>
              <article>
                <strong>Леонид Шевченко</strong>
              </article>
              <article>
                <strong>Евгения Белоусова</strong>
              </article>
              <article>
                <strong>Захар Веселов</strong>
              </article>
              <article>
                <strong>Владимир Ерошенко</strong>
              </article>
              <article>
                <strong>Дарья Демешко</strong>
              </article>
            </div>
          </section>
        </main>
